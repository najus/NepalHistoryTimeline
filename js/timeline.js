...
(function(){
	...
	TimeLine.prototype.addAuthors = function (item) {
		...
		if (item.authors.length > 0) {
			html += '<p><b>Author:</b> ' + item.authors.join(', ') + '</p>';
		}
	...
	TimeLine.prototype.addVideos = function (item) {
		...
		if (item.video_play.length > 0) {
			if (item.video_play[0].url) { // get only first item
				html += '<p>'+'<b>';
				html += '<i class="fa fa-youtube-play">';
				html += '<a href="'+item.video_play[0].url+'" class="venobox_custom" data-type="youtube" data-overlay="rgba(0,0,0,0.5)">Gameplay Video</a>';
				//html += '<a href="'+item.video_play[0].url+'" class="venobox_custom" data-type="youtube" data-overlay="rgba(0,0,0,0.5)">Gameplay Video</a>';
				html += '</i>';
				html += '</b>'+'</p>';
			}
